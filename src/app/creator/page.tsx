...
const { data, error } = await supabase
  .from('movies')
  .select('id, title, poster_url, view_count, admin_rating, rating, is_published, status, rejected_reason, created_at')
  ...
  rating:           m.rating,
  status:           m.status || (m.is_published ? 'published' : 'pending'),
  created_at:       m.created_at,
  rejection_reason: m.rejected_reason,
}))
  ...
  <div style={{
    ...
    <p style={{ fontSize: '0.72rem', color: '#f87171', fontWeight: 700, marginBottom: '0.15rem' }}>
      ✕ Rejected by Admin
    </p>
    <p style={{ fontSize: '0.75rem', color: 'rgba(248,113,113,0.8)' }}>
