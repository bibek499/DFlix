...
import {
  Film, Users, TrendingUp, Plus, Search, BarChart3, Eye, Star,
...
export default function AdminDashboard() {
  ...
  const init = async () => {
    try {
      ...
      // ── Check role in profiles table first ──
      ...
      // ── Determine role from any source ──
      const role = profileData?.role || userData?.role || metaRole || 'user'
      ...
      const mergedProfile = {
        ...
        ...profileData,
        role,
        full_name: profileData?.name || profileData?.full_name || userData?.name ||
...
const rejectFilm = async (film: PendingFilm) => {
  if (!rejectReason.trim()) { 
    showToast('Enter a rejection reason first.', 'err')
    return 
  }
        ...
        <div style={{ padding: '0.75rem', borderTop: '1px solid rgba(255,98,0,0.08)' }}>
          {sidebarOpen && user && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem', padding: '0.65rem', background: 'rgba(255,255,255,0.03)', borderRadius: 10, marginBottom: '0.6rem' }}>
              <div style={{ width: 34, height: 34, borderRadius: '50%', overflow: 'hidden', flexShrink: 0, border: '1px solid rgba(255,140,0,0.3)' }}>
                {profile?.avatar_url
                  ? <Image src={profile.avatar_url} alt="avatar" width={34} height={34} style={{ objectFit: 'cover', width: '100%', height: '100%' }} unoptimized />
                  : <div style={{ width: '100%', height: '100%', background: 'linear-gradient(135deg,#FF6200,#FFB733)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Bebas Neue,sans-serif', fontSize: '1rem', color: 'white' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <p style={{ fontSize: '0.68rem', color: '#FF8C00', textTransform: 'capitalize' }}>
                  {profile?.role || 'admin'}
                </p>
              </div>
            </div>
            ...
            <div>
              ...
              <p style={{ fontSize: '0.72rem', color: 'rgba(255,255,255,0.38)', lineHeight: 1 }}>
                {activeSection === 'pending'
                  ? `${stats.pendingCount} film${stats.pendingCount !== 1 ? 's' : ''} awaiting review`
        ...
        <div style={{ padding: 'clamp(1.25rem,3vh,2rem) clamp(1rem,3vw,2rem)', flex: 1 }}>
          ...
          {!isAdmin && (
            <div style={{ padding: '0.85rem 1.1rem', background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.22)', borderRadius: 12, marginBottom: '1.5rem', color: '#fbbf24', fontSize: '0.88rem', display: 'flex', alignItems: 'center', gap: '0.55rem' }}>
              ...
              You are viewing as a non-admin. Update your role in the profiles table to enable all features.
            </div>
          )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {filteredPending.map(film => (
                    <PendingCard
                      ...
                      onToggle={() => { setExpandedFilm(expandedFilm === film.id ? null : film.id); setRejectReason('') }}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                  ...
                  <div style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 16, padding: '1.35rem' }}>
                    ...
                    {[
                      ...
                      { label: 'Your Role',    value: profile?.role || 'admin', color: '#FF8C00' },
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', padding: '1.25rem' }}>
              ...
              {film.genre?.length ? (
                <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem' }}>
                    {film.genre.map(g => (
                      <span key={g} style={{ padding: '0.2rem 0.6rem', borderRadius: 9999, background: 'rgba(255,98,0,0.1)', border: '1px solid rgba(255,140,0,0.2)', color: 'rgba(255,183,51,0.9)', fontSize: '0.74rem', fontWeight: 600 }}>{g}</span>
                    ))}
                  </div>
                </div>
              ) : null}
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <button onClick={onApprove} disabled={!!actionLoading}
              ...
              {approving ? '⏳' : <CheckCircle style={{ width: 17, height: 17 }} />}
              Approve & Publish
            </button>
            <button onClick={onReject} disabled={!!actionLoading}
              ...
              {rejecting ? '⏳' : <XCircle style={{ width: 17, height: 17 }} />}
              Reject
            </button>
          </div>
        </div>
